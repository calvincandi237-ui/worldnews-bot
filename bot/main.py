...
def format_post(article: dict) -> str | None:
    ...
    prompt = (
        "Format this news article as a Telegram channel post following this EXACT structure:\n\n"
        "[HEADLINE IN CAPS]\n"
        "1-3 lines of factual summary\n"
        "💡 Why it matters: one concise sentence\n"
        f"{article['link']}\n\n"
        "Rules:\n"
        "- Headline must be ALL CAPS, max 12 words\n"
        "- Summary: factual, no fluff, 1-3 lines\n"
        "- 'Why it matters' line must start with exactly '💡 Why it matters:'\n"
        "- Last line is just the URL, no label\n"
        "- No hashtags, no extra emojis, no markdown symbols\n\n"
